...
from job_scanner.src.utils.logger_setup import start_logger
import logging
import time
import random
import requests
...
def scrape_linkedin_jobs(keyword: str, location: str, work_type: int, post_date: str, job_type: str, pages: int = 2, pull_min_time: float = 1.8, pull_max_time: float = 3.6) -> list[dict]:
    ...
    for page in range(pages):
        ...
        response = requests.get(lined_base_url, headers=headers, params=params)
        if response.status_code != 200:
            LOG.warning("LinkedIn blocked request (%s)", response.status_code)
            continue
